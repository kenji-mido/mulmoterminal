...
// `buildId` is the fork's, and it goes LAST so upstream's parameter order is untouched — its
// specs call this with three arguments. It names the CLIENT bundle this server hands out
// (config/build-id.ts); the browser records it on load and compares on every reconnect, so a tab
// left open through a rebuild can say so instead of quietly running yesterday's code.
export function mountConfigRoutes(app: Express, claudeCwd: string, onCwdPresetsChanged?: CwdPresetsChanged, buildId: string | null = null): void {
  ...
  app.get("/api/config", (_req, res) => {
    ...
    // silently do nothing.
    res.json({ ...configResponse(), home: os.homedir(), worktreesRoot: canonicalPath(worktreesRootDir()), buildId });
  });
