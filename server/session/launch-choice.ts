...
// environment, so they are validated here rather than trusted — each against the same id
// shape the config schema accepts for it, so the picker can never offer something this drops.
//
...
// The predicate travels with the parameter: a model id may carry the `[1m]` extended-context
// suffix and a provider id may not, so reading both with one rule would accept a provider the
// config schema then refuses.
const cleanParam = (params: URLSearchParams, name: string, usable: (value: string) => boolean): CleanParam => {
...
export function launchChoiceFromParams(params: URLSearchParams): DirModelChoice | undefined {
  const provider = cleanParam(params, "provider", isUsableProviderId);
  const model = cleanParam(params, "model", isUsableModelId);
