...
// Shape survives that; a Set does not. Same reasoning, and the same helpers, as the probe's ids
// (agents/probe-session.ts).
...
/** "4ead" for headless. Distinct from the probe's prefix so a listing filter, and a delete, can
 *  tell the two kinds apart even though both are ours. */
 ...
 *  a "delete this user's session" by a future caller passing the wrong variable — the same guard
 *  removeProbeTranscript carries, for the same reason. */
 ...
 *
 *  Unlike the probe's legacy sweep this needs no content test and no once-ever marker: every file
 *  it touches is addressed by a NAME only this server can mint, so there is no conversation it
 *  could mistake for ours. It reads no file contents at all — just the directory listing — which
 *  is why it can run on every boot. */
