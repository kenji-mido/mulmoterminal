...
describe("agent registry", () => {
  ...
  const originalGrokBin = process.env.GROK_BIN;
  afterEach(() => {
    ...
    restoreEnv("GROK_BIN", originalGrokBin);
  });
  ...
  it("resolves the grok adapter", () => {
    expect(getAgentAdapter("grok").kind).toBe("grok");
    expect(getAgentAdapter("grok")).toBe(grokAdapter);
  });
  ...
  it("reads each agent's binary from its env override, with a sensible default", () => {
    ...
    expect(grokAdapter.bin()).toBe("grok");
    ...
    expect(grokAdapter.bin()).toBe("/custom/grok");
  });
  ...
  it("exposes Claude's draft-ready marker but not the other agents (not wired yet)", () => {
    ...
    expect(getAgentAdapter("grok").draftReadyMarker).toBeUndefined();
  });
});
