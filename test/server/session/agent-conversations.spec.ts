...
// The log that maps a MulmoTerminal session to the conversation its agent is running — an agy
// conversation (#1096) or a codex rollout (#1418), in one format across two files. It is what
// makes a conversation reachable after a restart, it is written by several instances at once, and
// it is read by builds older and newer than the one that wrote it — so what a line means, and what
// an unusable one costs, is pinned here.
...
beforeAll(async () => {
  logDir = await mkdtemp(path.join(tmpdir(), "agent-conversation-log-"));
});
...
async function foldLog(contents: string): Promise<Map<string, AgentConversation>> {
  ...
  await forEachJsonlRecord(file, (parsed) => {
    const found = agentConversationRecord(parsed, isValidId);
    if (found) applyAgentConversation(conversations, found);
  });
  return conversations;
}

describe("agentConversationLine", () => {
  it("round-trips through the parser", async () => {
    expect((await foldLog(agentConversationLine(record()))).get(SESSION_A)).toEqual(record());
  });

  it("keeps a cwd that contains spaces — the reason this is JSON and not a split line", async () => {
    const line = agentConversationLine(record({ cwd: "/work/my project" }));
  ...
  it("ends its own line, so the next append cannot weld onto it", async () => {
    const log = `${agentConversationLine(record())}${agentConversationLine(record({ sessionId: SESSION_B, conversationId: CONVERSATION_B }))}`;
    expect([...(await foldLog(log)).keys()]).toEqual([SESSION_A, SESSION_B]);
  });
});

describe("agentConversationRecord", () => {
  it.each([
    ...
    expect(agentConversationRecord(parsed, isValidId)).toBeNull();
  });
  ...
  it("keeps a record whose startedAt is missing or unusable", () => {
    ...
    expect(agentConversationRecord(parsed, isValidId)).toEqual(record({ startedAt: 0 }));
  });
  ...
  it.each([
    ...
    ["a torn last line", `${agentConversationLine(record())}{"sessionId":"${SESSION_B}`],
  ...
  it("still keeps the good lines around a broken one", async () => {
    const log = `not json\n${agentConversationLine(record())}{"sessionId":"truncated`;
...
describe("applyAgentConversation", () => {
  it("lets the last line for a session win", async () => {
    const log = `${agentConversationLine(record({ cwd: "/work/old" }))}${agentConversationLine(record({ cwd: "/work/new" }))}`;
...
describe("hydrateAgentConversationInto", () => {
  it("fills what is missing", () => {
    const conversations = new Map<string, AgentConversation>();
    hydrateAgentConversationInto(conversations, new Set(), record());
  ...
  it("does not overwrite a session recorded while it was reading", () => {
    ...
    hydrateAgentConversationInto(conversations, new Set([SESSION_A]), record({ cwd: "/work/from-disk" }));
  ...
  it("still fills the OTHER sessions in the same file", () => {
    ...
    hydrateAgentConversationInto(conversations, written, record({ cwd: "/work/from-disk" }));
    hydrateAgentConversationInto(conversations, written, record({ sessionId: SESSION_B, conversationId: CONVERSATION_B }));
    expect(conversations.get(SESSION_A)?.cwd).toBe("/work/live");
    expect(conversations.get(SESSION_B)?.conversationId).toBe(CONVERSATION_B);
  });
});
...
describe("a hydrated log as agentResumeId's mappedId", () => {
  ...
  it("resumes the LAST conversation recorded for a session, not the first", async () => {
    const log = `${agentConversationLine(record())}${agentConversationLine(record({ conversationId: CONVERSATION_B }))}`;
    const conversations = await foldLog(log);
