...
同じリポで 2 体走らせても、互いのファイルを踏みません。

**→ [git worktree で作業を隔離する](worktree.html)** に、ブランチ名と分岐元・1 worktree に 1 セッション
という制約・閉じるときの片付け・プロジェクト設定の引き継ぎまで、通しでまとめてあります。
...
GitHub を開く、ビルドを走らせる）や**表示チップ**を足せます。

**→ [ヘッダーをカスタマイズする](header.html)** に、スクリーンショット付きで最初の 1 個から。
